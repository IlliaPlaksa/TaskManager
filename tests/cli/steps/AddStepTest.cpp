...
class AddStepTest : public ::testing::Test
{
protected:
...
TEST_F(AddStepTest, shouldReturnCommand)
{
    auto add_step = AddStep(dependency_);
    ...
    auto result = add_step.Execute(context);

    EXPECT_EQ(result.next_step, result_step);
    EXPECT_NE(result.command.get(), nullptr);
}

TEST_F(AddStepTest, shouldNotReturnCommand)
{
    auto add_step = AddStep(dependency_);
